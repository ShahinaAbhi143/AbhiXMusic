...
async def help(client: Client, message: Message):
    await message.reply_photo(
        photo=f"https://graph.org/file/1e0af186d0575a7d6a650.jpg",
        caption=f"""||Bʜᴀᴀɢ Bʜɪᴋᴀʀɪ Rᴇᴘᴏ Tᴏ Nᴀʜɪ Dᴜɴɢᴀ||""",
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        "🥀 Pᴀᴘᴀ Jɪ 🥀", url=f"https://t.me/imagine_iq"
                    )
                ]
            ]
        ),
...
async def add_allbot(client, message):
    ...
    if len(command_parts) != 2:
        await message.reply(
            "**⚠️ ɪɴᴠᴀʟɪᴅ ᴄᴏᴍᴍᴀɴᴅ ғᴏʀᴍᴀᴛ. ᴘʟᴇᴀsᴇ ᴜsᴇ ʟɪᴋᴇ » `/gadd @imagine_iq`**"
        )
        return
