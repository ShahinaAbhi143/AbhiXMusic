...
async def gemini_handler(client, message: Message):
    ...
    if len(message.command) > 1:
        ...
        await message.reply_text("Example: `/ask Who Is @ceo_of_secularism is AI?` or `!abhi What is the capital of India?`", quote=True)
        return
    ...
    try:
        ...
        if response and hasattr(response, 'text') and response.text:
            ...
            response_text = f"💡Fʀᴏᴍ: ||@ceo_of_secularism||\n\n" \
